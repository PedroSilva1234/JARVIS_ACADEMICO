...
# Cálculo da matriz inversa e a Fatoração de Cholesky
...
os fatores da fatoração LU com pivoteamento parcial da matriz A.
...
Então, devemos resolver os seguintes sistemas lineares:
...
Uma matriz A : n × n é simétrica se AT = A, ou seja aij = aji, para ∀i,j = 1, 2, . . . n.
...
Se A = GGT, na resolução do sistema linear Ax = b, teremos
...
# Exemplo: Determine a fatoração de Cholesky da matriz.
...
pode ser usada para verificar se uma matriz é simétrica e definida positiva; se o método falhar, a hipótese é falsa.

A fatoração de Cholesky requer a metade do número de operações efetuadas na fatoração LU.
