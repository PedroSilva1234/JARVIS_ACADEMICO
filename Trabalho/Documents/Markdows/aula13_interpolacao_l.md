...
Utilizada quando é necessário fazer estimativas de valores intermediários entre dados precisos.

# Exemplo

Um ve´ıculo de fabricação nacional, após vários testes, apresentou os resultados a seguir quando analisou-se o consumo de combust´ıvel de acordo com a velocidade média imposta ao ve´ıculo. Os testes foram realizados em rodovia em operação normal de tráfego, numa distância de 72 km.
...
aproximamos f(x) por um polinômio pn (x) de grau no máximo n tal que f(xi) = pn (xi), para i = 1, 2, · · · , n.

Lembrando: pn (x) = a0 + a1x + a2x² + · · · + an−1xn−1 + an xn em que ai, i = 0, 1, · · · , n são escalares.
...
Forma de Lagrange para o polinômio de interpolação

Sejam x0, x1, · · · , xn, distintos. Considere os polinômios Li(x) de grau n, para i = 1, 2, · · · , n, dado por:
...
o polinômio de interpolação da forma de Lagrange, sendo este polinômio de grau no máximo n e satisfazendo pn (xi) = f(xi), i = 0, 1, · · · , n.
...
Observação: f[x0, x1, · · · , xn] é simétrico nos argumentos. Por exemplo, para n = 2
...
a) Calcule f(1) usando a forma de Newton para o polinômio de interpolação utilizando todos os pontos da tabela.

Temos três pontos para interpolação, segue que, o polinômio é de grau no máximo 2 (n = 2). Então:
...
b) Calcule f(2) através de um polinômio que interpola f(x) nos pontos x0 = 0 e x1 = 3.
