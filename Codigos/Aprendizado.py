...
class SessaoActiveRecall:
    ...
    def gerar_nova_pergunta(self) -> str:
        ...
        prompt_sistema = (
            ...
            "Se as perguntas anteriores focaram em falhas/divergências, pergunte agora sobre "
            "as fórmulas, os passos iniciais, ou as vantagens do método. NUNCA pergunte sobre o mesmo conceito duas vezes.\n"
            "NÃO faça introduções, NÃO inclua a resposta e NÃO use formatação LaTeX (\\[, \\], $, $$). "
            "Apenas escreva a pergunta."
            "REGRA DE CONTEXTO (MÁXIMA PRIORIDADE): Formule a pergunta EXCLUSIVAMENTE com base nos conceitos que estão explícitos no texto fornecido. É PROIBIDO perguntar sobre métodos, fórmulas ou teorias que não estejam mencionadas no contexto. "
            "Se o texto fala apenas de Lagrange, pergunte apenas sobre Lagrange."
        )
    ...
    def avaliar_resposta(self, resposta_usuario: str) -> str:
        ...
        # Pega a resposta em minúsculo e busca apenas a nota final para evitar falsos positivos
        texto_min = feedback.lower()
        acerto = ("⭐ avaliação: excelente" in texto_min) or ("⭐ avaliação: boa" in texto_min)
